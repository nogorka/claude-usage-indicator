...
Текст панели (`panel_label`: бар, округление процента, порядок окон) обязан
совпадать бит-в-бит с рендером bash-хука statusLine на одном и том же
rate_limits — паритет держит гвард `tests/test_bar_parity.py`, прогоняющий
оба рендера на общем наборе фикстур. `effective_percent`/`is_stale` — логика
демона поверх уже сохранённого снимка, у bash-хука аналога нет, и гвард её
не проверяет.
"""
