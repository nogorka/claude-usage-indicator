...
# (перевод строки в т.ч.) и разделитель "·" её бы разломали. Решение
# контроллера: санировать при записи, а не полагаться на источник.
...
# бы один сегмент дважды). Решение контроллера: побеждает первое вхождение.
def dedup_by_key:
