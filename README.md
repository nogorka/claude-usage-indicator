...
(пусто или `~/.claude` дают `default`; иначе — имя каталога конфига без
префикса `claude-`, в нижнем регистре). Файл на профиль, а не общий файл —
...
сессию или систему целиком. Обе сессии — под разными аккаунтами — могут идти
рядом; какую из них использовать в моменте, решает человек, а не индикатор.
